...
async function initGame() {
    try {
        ...
        elements.apiStatus.textContent = '🤖 Supabase-Powered Mysteries';
...
async function loadUserProfile() {
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/game-api/profile/${gameState.userId}`, {
            headers: {
        ...
        elements.userHints.textContent = profile.total_hints || 0;
        elements.userStreak.textContent = profile.solve_streak || 0;
        elements.cognitiveStyle.textContent = profile.cognitiveStyle || 'New Detective';

        updateHintButtons(profile.total_hints || 0);
...
async function updateLiveFeed() {
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/game-api/stats`, {
            headers: {
                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                'Content-Type': 'application/json',
            }
...
async function startGame() {
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/generate-mystery?category=${gameState.selectedCategory}`, {
            headers: {
                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                'Content-Type': 'application/json',
            }
        ...
        gameState.totalClues = mystery.clues.length;
        gameState.allClues = mystery.clues;
...
async function submitGuess() {
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/game-api/guess`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                'Content-Type': 'application/json',
...
async function useHint(hintType) {
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/game-api/hint/use`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                'Content-Type': 'application/json',
...
async function revealAnswer() {
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/game-api/reveal`, {
            headers: {
                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                'Content-Type': 'application/json',
            }
...
async function showProfile() {
    try {
        ...
        elements.profileGames.textContent = profile.total_games;
        elements.profileSolves.textContent = profile.total_solves;
        elements.profileStreak.textContent = profile.solve_streak;
        elements.profileAvgClues.textContent = profile.average_clues_used;
